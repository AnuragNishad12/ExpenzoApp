...
import com.example.expenzo.Model.FetchOtherCurrentDataResponse30days
import com.example.expenzo.Repository.Fetch30DayDataRepo
import kotlinx.coroutines.launch

class Fetch30daysTransViewModel : ViewModel() {
    private val repository = Fetch30DayDataRepo()

    val error = MutableLiveData<String?>()
    val transactions = MutableLiveData<List<FetchOtherCurrentDataResponse30days>>()
    val totalAmount = MutableLiveData<Double>()
    val highestTransaction = MutableLiveData<FetchOtherCurrentDataResponse30days?>()
    ...
    fun showTransaction30daysVm(data: FetchCurrentDayDataModel30days) {
            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    for (txn in userDataList) {
                        ...
                        receiverTotals[receiver] =
                            receiverTotals.getOrDefault(receiver, 0.0) + amount
                    }
                    ...
                    for (txn in userDataList) {
                        val date =
                            txn.Date ?: continue // Assuming your transaction has a Date field
                        val amount = txn.Amount?.toDoubleOrNull() ?: 0.0
                    ...
                    Log.d(
                        "Analytics",
                        "Total = ₹$total, Max = ₹${maxTxn?.Amount}, Top Receiver = ${mostFrequent?.key}"
                    )
